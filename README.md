...
Copy 于https://github.com/gyf-dev/ImmersionBar  但是他的只支持了api 29  aar 包在/libs/immersionbar-library-release.aar
打包封装了一个 aar 支持到AndroidX 可以直接在项目引用aar就行
添加方式 在app的build.gradle下添加这几处 
android {    
    sourceSets.main {
...
dependencies {
    implementation fileTree(include: ['*.jar'], dir: 'libs')  //(2)
    implementation(name: 'immersionbar-library-release', ext: 'aar') // //(3)自己做的浸入式状态栏
